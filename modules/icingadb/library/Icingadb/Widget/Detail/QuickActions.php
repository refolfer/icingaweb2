...
use Icinga\Module\Icingadb\Common\Auth as ObjectAuth;
...
class QuickActions extends BaseHtmlElement
{
    ...
    protected function assembleAssignmentAction(string $label, string $title)
    {
        $link = Html::tag(
            'a',
            [
    ...
    protected function isCriticalProblem($object): bool
    {
        return $object->state->is_problem
            && (
                (int) $object->state->hard_state === 2
                || (int) $object->state->soft_state === 2
            );
    }
